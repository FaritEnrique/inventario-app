...
const OrdenesCompraPage = () => {
  const { user } = useAuth();
  const { loading, obtenerBandejaAprobacionOrdenCompra, obtenerOrdenesCompra } =
    useOrdenesCompra();
  ...
  const [filters, setFilters] = useState(() =>
    buildDefaultFilters(currentView),
  );
    ...
    return (
      <div className="mx-auto max-w-5xl p-6">
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
          Tu usuario no tiene acceso disponible para consultar ordenes de
          compra.
        </div>
      </div>
    );
  ...
  if (currentView === "listado" && !canViewList) {
    return (
      <div className="mx-auto max-w-5xl p-6">
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
          Tu usuario no tiene acceso al listado general de ordenes de compra
          para este contexto activo.
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      ...
      >
        <input
          ...
          placeholder="Buscar por código, proveedor o RUC"
        ...
        {currentView === "listado" ? (
          <>
            ...
            >
              ...
              <option value="PARCIALMENTE_RECIBIDA">
                Parcialmente recibida
              </option>
              <option value="COMPLETAMENTE_RECIBIDA">
          ...
          >
            ...
            <option value="GERENCIA_ADMINISTRACION">
                  ...
                  {rows.length > 0 ? (
                    rows.map((ordenCompra) => {
                      const requerimientos =
                        buildRequerimientoRefs(ordenCompra);
                      return (

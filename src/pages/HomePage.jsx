...
const HomePage = () => {
  ...
  useEffect(() => {
    const verificarUsuarios = async () => {
      try {
        ...
        setUsuariosExisten(data.count !== 0);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isPasswordPolicyValid(nuevoUsuario.password)) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white p-8 text-center shadow-lg">
        ...
        <Link
          to="/login"
          className="inline-block rounded-xl bg-blue-600 px-6 py-3 text-lg font-semibold text-white transition duration-300 hover:bg-blue-700"
        >
          Ingresar al sistema
        </Link>
      </div>
